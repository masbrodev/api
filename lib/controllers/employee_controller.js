...
let saveEmployee = (req, res, next) => {
    ...
    let employee = new Employee(data.id_buku, data.judul, data.jenis, data.keterangan);
...
let updateEmployee = (req, res, next) => {
  ...
  let employee = new Employee(id_buku, data.judul, data.jenis, data.keterangan);
  let employeeRepo = new EmployeeRepository(db);
...
let getEmployee = (req, res, next) => {
  ...
  let id_buku = req.params.id_buku;
  let employeeRepo = new EmployeeRepository(db);
  employeeRepo.findOne(id_buku, result => {
